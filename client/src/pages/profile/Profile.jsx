...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      const res = await axios(`${API_URL}/users/single?username=${username}`);
      setUser(res.data)
    }
  ...
  const gerFriends = async () => {
    ...
    try {
      if (currentUser?._id) {
        const { data } = await axios.get(`${API_URL}/users/friends/${currentUser?._id}`, config)
        setFriendsList(data)
      }
    } catch (err) {
      console.log(err);
  ...
  const unfriendHandler = async (e) => {
    ...
    try {
      const { data } = await axios.put(`${API_URL}/users/friends/${targetUserId}/unfriend`, {}, config)
  ...
  const gerFriendsRequest = async () => {
    ...
    try {
      if (user?._id) {
        const { data } = await axios.get(`${API_URL}/users/friends/view-friend-request/${user?._id}`, config)
  ...
  const confirmRequestHandler = async (e) => {
    const targetUserId = e.target.value;
    try {
      const { data } = await axios.put(`${API_URL}/users/friends/${targetUserId}/confirm-request`, {}, config)
      if (data.status === "success") {
        gerFriendsRequest();
      }
  ...
  const deleteRequestHandler = async (e) => {
    const targetUserId = e.target.value;
    try {
      const { data } = await axios.put(`${API_URL}/users/friends/${targetUserId}/delete-request`, {}, config)
      if (data.status === "success") {
        gerFriendsRequest();
      }
  ...
  const gerFriendsSentRequest = async () => {
    ...
    try {
      if (currentUser?._id) {
        const { data } = await axios.get(`${API_URL}/users/friends/view-sent-req/${currentUser?._id}`, config)
  ...
  const cancelRequestHandler = async (e) => {
    ...
    const targetUserId = e.target.value;
    try {
      const { data } = await axios.put(`${API_URL}/users/friends/${targetUserId}/cancel-request`, {}, config)
      if (data.status === "success") {
        // setLoading(false)
        // loading = false;
