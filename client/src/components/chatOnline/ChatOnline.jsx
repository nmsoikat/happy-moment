...
function ChatOnline({onlineUsers, currentUserId, setCurrentChat}) {
  ...
  useEffect(() => {
    const getFriends = async () => {
      const res = await axios.get(`${API_URL}/users/friends/${currentUserId}`, config)
  ...
  const handleClick = async (onlineF) => {
    try {
      const res = await axios.get(`${API_URL}/conversation/find/${currentUserId}/${onlineF._id}`, config);
